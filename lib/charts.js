...
export function buildTCCAmpsGeometry(curves, evalA) {
  ...
  const yTicks = [0.01, 0.1, 1, 10, 100].map(v => ({ v, y: yLog(v), label: `${v >= 1 ? v : v.toFixed(2)}s` }));
  // timeFn(I): fungsi generik jam-vs-arus per kurva. Kalau tidak diberikan, jatuh ke kurva IDMT (curveKey+dial) seperti sebelumnya.
  ...
  const paths = curves.map(cv => {
    ...
    const tEval = Math.max(yMin, Math.min(yMax, getT(cv, evalA)));
    return { d, color: cv.color, dashed: !!cv.dashed, dot: { x: xLog(evalA), y: yLog(tEval) } };
  });
